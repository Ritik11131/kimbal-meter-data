...
router.post("/", authenticate, authorize([MODULES.METER]), enforceEntityAccessQuery("entityId"), validate(createMeterSchema), (req, res, next) => {
  meterController.create(req, res).catch(next)
})

router.patch("/:id", authenticate, authorize([MODULES.METER]), validateUUIDParams(["id"]), enforceResourceEntityAccess("meter"), validate(updateMeterSchema), (req, res, next) => {
  meterController.update(req, res).catch(next)
})

router.delete("/:id", authenticate, authorize([MODULES.METER]), validateUUIDParams(["id"]), enforceResourceEntityAccess("meter"), (req, res, next) => {
