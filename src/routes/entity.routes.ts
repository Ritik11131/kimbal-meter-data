...
router.get("/:id", authenticate, authorizeRead([MODULES.ENTITY]), enforceEntityAccess(), (req, res, next) => entityController.getById(req, res).catch(next))
router.get("/:id/hierarchy", authenticate, authorizeRead([MODULES.ENTITY]), enforceEntityAccess(), (req, res, next) => entityController.getHierarchy(req, res).catch(next))
...
router.patch("/:id", authenticate, authorize([MODULES.ENTITY]), enforceEntityAccess(), validate(updateEntitySchema), (req, res, next) => {
  entityController.update(req, res).catch(next)
})

router.delete("/:id", authenticate, authorize([MODULES.ENTITY]), enforceEntityAccess(), (req, res, next) => {
